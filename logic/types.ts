export const LOGIC_TYPES = {
    IWebServer: Symbol.for('IWebServer'),
    IMapper: Symbol.for('IMapper'),
    IStore: Symbol.for('IStore'),
    ILogger: Symbol.for('ILogger'),
    IServer: Symbol.for('IServer'),
    IEnvironment: Symbol.for('IEnvironment'),
    IRunner: Symbol.for('IRunner'),
    IFileManager: Symbol.for('IFileManager'),
    ISheetIdentifier: Symbol.for('ISheetIdentifier'),
    IExcelProcessor: Symbol.for('IExcelProcessor'),
    IDateFromatAnalyzer: Symbol.for('IDateFromatAnalyzer'),
    IIntegration: Symbol.for('IIntegration')
}
